 ...
 */
public class LogisticsNoticeActivity extends BaseActivity<NoticePresenter> implements INoticeView,OnLoadMoreListener {
    ...
    public void isGetOrderNoticeSuc(OrderNoticeBean data) {
        if (isLoadMore) {
            ...
            if (!mNoticeAdapter.notifyChangeData(data.getList())) {
                mNoticeAdapter.setEmptyView(getLoadSirView(R.mipmap.ic_none_list_data, "您还没有物流消息", false,
                        null));
                smartRefresh.setEnableLoadMore(false);
            } else {
