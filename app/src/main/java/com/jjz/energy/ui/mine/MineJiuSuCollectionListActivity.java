...
import com.jjz.energy.adapter.MineJiuSuCollectionAdpter;
 ...
 */
public class MineJiuSuCollectionListActivity extends BaseActivity<JiuSuShopPresenter> implements IJiuSuShopView, OnRefreshLoadMoreListener {
    ...
    private MineJiuSuCollectionAdpter mAdapter;

    @Override
    protected void initView() {
        tvToolbarTitle.setText("本店收款记录");
        //绑定刷新和加载
        smartRefresh.setOnRefreshLoadMoreListener(this);
        mAdapter = new MineJiuSuCollectionAdpter(R.layout.item_jiusu_shopping, new ArrayList<>());
        rvShoppingList.setLayoutManager(new LinearLayoutManager(mContext));
        rvShoppingList.setAdapter(mAdapter);
        //跳转详情页面
        mAdapter.setOnItemClickListener((adapter, view, position) -> {
            startActivity(new Intent(mContext, MineJiuSuCollectionDetailsActivity.class).putExtra(Constant.ORDER_SN, mAdapter.getItem(position).getOrder_sn()));
        });
        getData(false);
    }
    ...
    private void getData(boolean isLoadMore){
        ...
        mPresenter.getJiuSuCollectionList(PacketUtil.getRequestPacket(Utils.stringToMap("page",mPage+"")),isLoadMore);
    }
    ...
    public void isGetJiusuShoppingListSuc(JiuSuShoppingBean data) {
        if (isLoadMore) {
            ...
            if (!mAdapter.notifyChangeData(data.getList())) {
                mAdapter.setEmptyView(getLoadSirView(R.mipmap.ic_none_data, "本店还未有人消费", false,
                        null));
                smartRefresh.setEnableLoadMore(false);
            } else {
                //有数据就开启加载更多
