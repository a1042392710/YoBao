...
//        ObjectAnimator.ofPropertyValuesHolder(view, pvhX, pvhY, pvhZ).setDuration(200).start();
    ...
    public void destory() {
        ...
        if (year_pv != null) {
            year_pv.stopTimerAndHandle();
        }
        if (month_pv != null) {
            month_pv.stopTimerAndHandle();
        }
        if (day_pv != null) {
            day_pv.stopTimerAndHandle();
        }
        if (hour_pv != null) {
            hour_pv.stopTimerAndHandle();
        }
        if (minute_pv != null) {
            minute_pv.stopTimerAndHandle();
        }
